...
titanic_bag_wfs <- 
  workflow_set(
    preproc = list(rec = titanic_bagged_rec,
...
With a grid of size $5^5=15625$ and relatively expensive models to fit, we will use tune_racing_anova to speed up the process. Tune_racing_anova is a method where is set of parameters is initially screened and obviously inefficient sets are discarded. Another option could have been to use a spacefilling grid instead of a regular grid.
