...
titanic_knn_best_parm
```
So the best option is kNN with n=6, rank weight function and L2-distance.
...
```
We see that the best performing model uses a Polynomial kernel.
...
titanic_svm_best_parm
```
So the best option is SVM with 2nd degree Polynomial kernel with cost parameter 3.69e-3.
...
test2 %>% 
  ...
  summarise_all(~ sum(is.na(.)))
```
Only the embarked column in the training data contains missing values. These will be imputed by the mode in the preprocessing step.
...
```{r}
imp_folds <- vfold_cv(train2,strata = Survived,repeats = 5)
```

We will fit the models that did best for the simple models; C5.0 and bagged C5.0.
```{r}
imp_c50 <- 
  decision_tree(min_n=tune()) %>% 
  set_engine("C5.0") %>% 
...
imp_workflowset_c50 <- 
  workflow_set(
    ...
    models = list(bag=imp_c50))

imp_workflowset_bc50 <- 
  workflow_set(
    preproc = list(rec = imp_rec),
    models = list(bag=imp_bc50))
```
...
```
```{r}
imp_bc50_grid <- tibble(min_n=c(4,8,11,14,18))
...
```
```{r}
imp_results<- bind_rows(imp_results_c50,imp_results_bc50)
```

Let us have a look at the results:
```{r}
autoplot(imp_results,rank_metric="accuracy") +
  geom_text(aes(y=0.65,label= wflow_id), angle=90) +
  lims(y=c(0.4,0.9)) +
  theme(legend.position = "none")
```
It seems that with regard to both accuracy and AUC, the bagged C5.0 model performs best.

Let us find the best tuning parameters with respect to accuracy:
```{r}
imp_best_parm <-
   imp_results %>% 
   extract_workflow_set_result("rec_bag") %>% 
   select_best(metric="accuracy")
imp_best_parm
```
We see that the best results are obtained with a minimum node size when splitting of 8.

We can now define best workflow and fit to the training data
```{r}
imp_best_wf <- imp_results %>% 
  extract_workflow("rec_bag") %>% 
  finalize_workflow(imp_best_parm)

imp_best_mod <- fit(imp_best_wf,train2)

imp_pred <- predict(imp_best_mod,new_data=test2)
```
We are now ready to write the predictions to a file and upload to Kaggle:
```{r}
cbind(PassengerId = titanic_test$PassengerId,imp_pred) %>% 
  rename(Survived = .pred_class) %>% 
  write_csv("./titanic_imp_pred.csv")
```
